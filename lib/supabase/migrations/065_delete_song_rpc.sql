-- RPC functions for deleting songs from setlists and catalog
-- Uses SECURITY DEFINER to bypass RLS policies
-- Handles position reordering in a constraint-safe way
...
COMMENT ON FUNCTION delete_song_from_setlist IS 
  'Removes a song from a specific setlist and safely reorders remaining positions.';
...
COMMENT ON FUNCTION delete_song_from_catalog IS 
  'Completely removes a song from a band: deletes from all setlists (safely reordering positions) and removes the song record.';


-- =============================================================================
-- DISABLE THE OLD TRIGGER THAT CAUSES ISSUES
-- =============================================================================
-- The old reorder_setlist_positions_on_delete trigger causes unique constraint
-- violations because it tries to update positions in a non-safe way.
-- We disable it since our RPC functions now handle reordering properly.
