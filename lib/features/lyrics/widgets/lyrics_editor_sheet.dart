...
class _LyricsEditorSheetState extends State<_LyricsEditorSheet>
  ...
  /// Per-block highlight overrides.  Key = block index (0-based).
  /// Blocks are separated by double-newlines in the text.
  ...
  void initState() {
    ...
    if (data != null && data.isNotEmpty) {
      ...
      // plainText joins blocks with '\n\n', so we walk through and map
      // each block's highlight onto all the lines it spans.
      int lineIdx = 0;
      for (var i = 0; i < data.blocks.length; i++) {
        ...
        lineIdx += lineCount;
        // Account for the '\n\n' separator between blocks (adds 2 lines:
        // the empty line between blocks)
        if (i < data.blocks.length - 1) {
          lineIdx += 2; // two newlines = skip past the blank separator line
        }
      }
    } else {
  ...
  void _onSelectionChanged() {
    ...
    if (sel == _prevSelection) return;
    _prevSelection = sel;
    // Rebuild so the color-preset chips reflect the current block's highlight.
    setState(() {});
  }
