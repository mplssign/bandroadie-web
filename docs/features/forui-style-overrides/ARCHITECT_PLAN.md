...
- `lib/features/setlists/widgets/song_lookup_overlay.dart` (search)
...
✅ Automated unit/widget tests for every restored prop following test/components/ui/ pattern
...
**Rationale:** The prior Forui swap cycle (PR #145) used 108 automated widget tests as its real acceptance gate—manual testing is not feasible in the Engineer/QA pipeline (no stable browser auth, no device access). This cycle follows the same pattern established in `test/components/ui/`.

**Scope:** One test per restored prop minimum, covering both API mapping and rendering verification.

**Unit Test Cases (Prop Pass-Through):**

1. **app_text_field_test.dart:**
   - Assert `onSubmitted` parameter maps to FTextField's `onSubmit` and fires callback
   - Assert `textInputAction` passes through to FTextField
   - Assert `autofocus` passes through to FTextField
   - Assert `readOnly` passes through to FTextField
   - Assert `textAlign`, `textCapitalization`, `inputFormatters`, `autofillHints` pass through
   - Assert `maxLength`, `minLines` pass through

2. **app_text_field_test.dart (Builder Pattern):**
   - Assert `prefixIcon` widget wraps in `prefixBuilder` lambda
   - Assert `suffixIcon` widget wraps in `suffixBuilder` lambda
   - Assert prefixBuilder calls FTextField.prefixIconBuilder static helper

3. **app_text_form_field_test.dart:**
   - Same as AppTextField (shares prop list)
   - Assert validator and onSaved still work with restored props

4. **app_button_test.dart:**
   - Assert `backgroundColor` constructs FButtonStyleDelta with decoration
   - Assert `padding` constructs FButtonStyleDelta with contentStyle
   - Assert `borderRadius` constructs FButtonStyleDelta with decoration

5. **app_switch_test.dart:**
   - Assert `activeColor` constructs FSwitchStyleDelta with trackColor/thumbColor

6. **app_checkbox_test.dart:**
   - Assert `activeColor` constructs FCheckboxStyleDelta with decoration

7. **app_card_test.dart:**
   - Assert `padding` constructs FCardStyleDelta
   - Assert `elevation` constructs FCardStyleDelta with decoration shadows

**Widget Test Cases (Rendering Verification):**

1. Render AppButton with `backgroundColor: Colors.red`, assert FButton receives FButtonStyleDelta
2. Render AppTextField with `prefixIcon: Icon(Icons.search)`, assert icon appears in widget tree via find.byIcon
3. Render AppTextField with `onSubmitted: callback`, tap submit action, assert callback fires
4. Render AppCard with `padding: EdgeInsets.all(24)`, assert FCardStyleDelta padding applied
5. Render AppSwitch with `activeColor: Colors.green`, assert FSwitchStyleDelta trackColor override

**Priority:** **REQUIRED** — These tests are the acceptance gate for this cycle.

**Test Count Target:** Minimum 20 new test cases across 7 wrapper test files (following 108-test pattern from prior cycle).

### 8.2 Manual Testing (OPTIONAL)

**Scope:** Post-QA smoke testing for Tony to run locally if desired.

**Suggested Manual Test Plan:**

1. **Search Fields (prefixIcon/suffixIcon):**
   - Test `lib/features/contacts/widgets/az_search_field.dart` - magnifying glass icon
   - Test `lib/features/setlists/setlist_detail_screen.dart` search bar
   - Test `lib/features/setlists/widgets/song_lookup_overlay.dart` search bar
...
**Note:** Manual testing is NOT a QA gate criterion—automated tests are authoritative.
...
- **Validation:** Automated tests for onSubmitted→onSubmit mapping, textInputAction pass-through, autofocus, readOnly, etc.

**Phase 2: Category C (Builder Pattern)**

- **Duration:** 3 hours
- **Scope:** Restore prefixIcon/suffixIcon via prefixBuilder/suffixBuilder
- **Validation:** Widget tests rendering search icons, unit tests for builder lambda construction

**Phase 3: Category D (StyleDelta - Simple)**

- **Duration:** 4 hours
- **Scope:** Restore backgroundColor, padding on AppButton via FButtonStyleDelta
- **Validation:** Unit tests for StyleDelta construction, widget tests for decoration rendering

**Phase 4: Category D (StyleDelta - Complex)**

- **Duration:** 4 hours
- **Scope:** Restore activeColor on AppSwitch/AppCheckbox, padding/elevation on AppCard
- **Validation:** Automated tests for FSwitchStyleDelta, FCheckboxStyleDelta, FCardStyleDelta
...
**Total Estimated Duration:** 17 hours (includes test authoring time)
...
✅ All Category A+B functional props restored with automated tests (onSubmitted, textInputAction, etc.)  
✅ All Category C builder props restored with automated tests (prefixIcon, suffixIcon)  
✅ Category D StyleDelta props restored with automated tests for AppButton, AppSwitch, AppCheckbox, AppCard  
✅ README.md "Props Ignored in Preview" sections removed  
✅ Minimum 20 automated tests added to test/components/ui/ following prior cycle's 108-test pattern  
✅ Zero visual regressions in existing Forui appearance

### 10.2 Quality Gates

- **No compiler errors** in wrapper implementations
- **All automated tests pass** in test/components/ui/ (REQUIRED acceptance gate)
- **No runtime exceptions** in widget tests
...
- **QA_REPORT.md** - Automated test results (20+ tests), coverage report, regression checklist
...
- Automated tests are the acceptance gate (following prior cycle's 108-test pattern)
