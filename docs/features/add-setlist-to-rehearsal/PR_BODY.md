...
QA recorded no Cycle 3 findings. Create-mode coverage confirms that Location is required: selecting a setlist alone intentionally leaves `Add Rehearsal` disabled, while entering Location enables it and setlist changes preserve that enabled state. The earlier non-blocking test-harness size warning was not worsened.
