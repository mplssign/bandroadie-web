...
- They have in-flight guards (`_isItemReorderInFlight`, `_itemReorderPendingAfterFlight`) that prevent UNIQUE constraint violations during rapid consecutive drags *within* a single session
...
| Setlists / Catalog | **affected** — fixes song reorder persistence in setlist detail screen |
...
Confirm which reorder path is actively used:
- Mixed-items path: `_handleItemReorder` (line 979) → `persistItemReorder()`
- Legacy songs-only path: `_handleReorder` (line 675) → `persistReorder()`

Both share `_reorderDebounceTimer`. The fix must handle whichever path set the timer.
...
**Note:** This calls `persistItemReorder()` (the actively-used mixed-items path). The legacy `persistReorder()` path shares the same timer and will also be flushed if active. If investigation in Task 2 reveals separate timers or paths, adjust accordingly.
...
- Which dispose() implementation was used (fire-and-forget with `unawaited`)
- Confirmation that both reorder paths are covered by the single timer flush
