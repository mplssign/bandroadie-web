...
Located in `supabase/functions/`. All 11 deployed functions:
...
| `send-push` | ❌ | FCM HTTP v1 push delivery (webhook-triggered, older arch) |
| `deliver-notifications` | ❌ | Batch push delivery — pg_cron every 5 min (current arch) |
