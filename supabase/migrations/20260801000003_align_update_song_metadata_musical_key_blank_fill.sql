-- Align musical_key fill condition to handle blank/whitespace sentinels.
-- Updates only the musical_key CASE branch to fill when current key is NULL, empty, or whitespace-only.
-- Required for bug/enrich-zero-fields-updated.
...
BEGIN
  ...
  SET
    ...
    -- CHANGED: Fill missing only, now includes blank/whitespace sentinels
...
COMMENT ON FUNCTION update_song_metadata IS
  'Update song metadata including musical key and duration. BPM only updates when currently NULL. Duration only updates when currently 0 (non-overwrite). Musical key updates when NULL, empty, or whitespace-only. SECURITY DEFINER to bypass RLS for legacy songs with NULL band_id.';
