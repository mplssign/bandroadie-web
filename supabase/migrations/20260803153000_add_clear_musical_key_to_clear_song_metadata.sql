-- Extend clear_song_metadata to support clearing musical key.
-- Keeps SECURITY DEFINER access pattern for legacy songs with NULL band_id.
...
GRANT EXECUTE ON FUNCTION clear_song_metadata(UUID, UUID, BOOLEAN, BOOLEAN, BOOLEAN, BOOLEAN)
TO authenticated;

COMMENT ON FUNCTION clear_song_metadata IS
  'Clears selected song metadata fields using boolean flags (bpm, duration, tuning, musical_key). SECURITY DEFINER to bypass RLS for legacy songs with NULL band_id.';
